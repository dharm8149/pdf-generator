...
create TABLE if not exists datad(pid integer PRIMARY KEY AUTOINCREMENT,name text,quan real,reta real,whol real,purc real);
...
create table if not exists remove_list(sid integer primary key  AUTOINCREMENT,name text,date Date,status char(1));
...
async function uQname(query){
 ...
 //////console.log(data)
// data=JSON.parse(data)
var check="select * from save_list where name='"+query+"' collate nocase";
var data1=await select(check)
//console.log(data1)
data1=JSON.parse(data1)
if(data1.msg!=''||query==""||query==" "){
